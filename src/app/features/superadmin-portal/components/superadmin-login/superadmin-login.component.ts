...
export class SuperadminLoginComponent {
  ...
  private afterLoginPath(): string {
    ...
    if (returnUrl?.startsWith('/superadmin')) return returnUrl;
  ...
  private readError(err: unknown): string {
    ...
    if (status === 0 || status === 500) {
      return 'Server error — ensure the backend is running on port 8000.';
    }
  ...
  login() {
    ...
    this.auth.superadminLogin(this.username, this.password).subscribe({
      next: () => {
